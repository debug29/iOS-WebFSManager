...
@implementation FCVideoPlayerView
{
    id playbackObserver;
...
-(void) setupConstraints
{
    ...
    self.closeBut.keepLeftInset.equal = KeepRequired(5);
    self.closeBut.keepTopInset.equal = KeepRequired(5);
    ...
    self.playBackTotalTime.keepRightInset.equal = KeepRequired(5);
...
-(void)initializePlayer:(CGRect)frame
{
    ...
    self.closeBut = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    self.closeBut.frame = CGRectMake(1*frameWidth/240, 1*frameHeight/160, 10*frameWidth/240, 10*frameHeight/160);
    [self.closeBut addTarget:self action:@selector(zoomButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    [self.closeBut setBackgroundImage:[UIImage imageNamed:@"closeButNormal.png"] forState:UIControlStateNormal];
    [self.closeBut setBackgroundImage:[UIImage imageNamed:@"closeButPress.png"] forState:UIControlStateHighlighted];
    self.closeBut.layer.opacity = 0;
...
-(void)zoomButtonPressed:(UIButton*)sender {
    UIViewController *temp = (UIViewController*)self.delegate;
...
-(void) showHud:(BOOL)show
{
    ...
    if(show) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            weakself.closeBut.layer.opacity = 0;
            viewIsShowing = show;
        }];
    } else {
        CGRect frame = self.playerHudBottom.frame;
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            weakself.closeBut.layer.opacity = 1;
            viewIsShowing = show;
        }];
    }
}
